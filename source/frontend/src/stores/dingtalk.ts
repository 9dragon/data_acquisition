...
export const useDingTalkStore = defineStore('dingtalk', () => {
  ...
  function getLocation(): Promise<{ latitude: number; longitude: number; address?: string }> {
    return new Promise((resolve, reject) => {
      if (isDingTalkEnv.value && (window as any).dd?.device?.geolocation) {
        // 使用钉钉定位
        ;(window as any).dd.device.geolocation.get({
          onSuccess: (result: any) => {
            resolve({
